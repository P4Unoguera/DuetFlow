...
// This script is attached to a platform that detects when Player2 enters or exits a trigger collider
public class Platform2Trigger : MonoBehaviour
{
    ...
    {
        // If the entering collider has the tag "Player2"...
        if (other.tag == "Player2")
        {
            // Mark Player2 as ready in the GameManager
            GameManager.Instance.player2Ready = true;
            // Change the platform's color to red to indicate readiness
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.tag == "Player2")
        {
            // Mark Player2 as not ready in the GameManager
